...
$keg = mysqli_fetch_array(mysqli_query($connect, "select * from tb_kegiatan where id_kegiatan='$id'"));
...
if(isset($_POST['submit'])){
	if($judul == ""){
        ...
        if(empty($foto)){ //tidak mengubah foto
            $sql = mysqli_query($connect, "update tb_kegiatan set judul='$judul', deskripsi_kegiatan='$deskripsi', id_kategori='$kategori' where id_kegiatan='$id'");
            if($sql){
                echo "<script>alert('Edit Berhasil');document.location='kegiatan_admin.php'</script>";
            }else{
                $gambar_error = "<span style='color:red;'>Terjadi kesalahan, silahkan coba lagi</span>";
            }
        }else{
            move_uploaded_file($foto, '../img/' . $namabaru);
            $sql = mysqli_query($connect, "update tb_kegiatan set judul='$judul', deskripsi_kegiatan='$deskripsi', id_kategori='$kategori', gambar='$namafoto' where id_kegiatan='$id'");
            if($sql){
                echo "<script>alert('Edit Berhasil');document.location='kegiatan_admin.php'</script>";
            }else{
                $gambar_error = "<span style='color:red;'>Terjadi kesalahan, silahkan coba lagi</span>";
...
<body>
    ...
    <div class="kegiatan_admin">
		<div class="container_admin">
            ...
            <td>
                <input type="text" name="judul" placeholder="Masukkan Judul" class="input" value="<?= $keg['judul'];?>">
                <?= $judul_error;?>
            </td>
            </tr>
            <tr>
            ...
            <td>
                <textarea name="deskripsi" rows="4" cols="40" placeholder="Deskripsi Kegiatan"><?= $keg['deskripsi_kegiatan'];?></textarea>
                <?= $deskripsi_error;?>
            </td>
            </tr>
            <tr>
